...
```text
Registos Arquivo.pt
  → variáveis digitais (Total_Arquivo_Captures, capture_density, digital_decay_rate)
...
- **Optimização de hiperparâmetros:** Optuna (Bayesian search)
...
> O objectivo não é apresentar uma previsão infalível, mas demonstrar que a memória digital arquivada contém **sinal territorial mensurável**.
...
- `cdx_checkpoint.json` — ficheiro interno de checkpoint; intencionalmente não exposto no site público.
